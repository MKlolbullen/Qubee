...
use crate::identity::identity_key::{
    DeviceId, DeviceKey, DevicePublicKey, HybridSignature, IdentityId, IdentityKey, IdentityKeyPair,
};
...
pub trait KeyDistributionServer {
    ...
    fn get_prekey_bundle(
        &self,
        identity_id: &IdentityId,
        device_id: &DeviceId,
    ) -> Result<PreKeyBundle>;
    fn remove_one_time_prekey(
        &mut self,
        identity_id: &IdentityId,
        device_id: &DeviceId,
        prekey_id: u32,
...
impl SignalProtocol {
    ...
    pub fn generate_one_time_prekeys(&mut self, count: usize) -> Result<Vec<OneTimePreKey>> {
        ...
        for _ in 0..count {
            ...
            self.one_time_prekeys
                .insert(prekey_id, one_time_prekey.clone());
    ...
    pub fn create_prekey_bundle(&self) -> Result<PreKeyBundle> {
        ...
        let one_time_prekey = self.one_time_prekeys.values().next().cloned();

        let bundle_timestamp = SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs();
    ...
    pub fn initiate_key_exchange(&self, remote_bundle: &PreKeyBundle) -> Result<KeyExchangeResult> {
    ...
    pub fn respond_to_key_exchange(
        ...
        let shared_secrets =
            self.reconstruct_3dh_key_exchange(initiator_device, used_one_time_key)?;
    ...
    fn verify_prekey_bundle(&self, bundle: &PreKeyBundle) -> Result<()> {
        ...
        let signature_data =
            self.serialize_device_key_for_signing(&bundle.signed_prekey.device_public_key)?;

        let signature_valid = bundle
            .identity_key
            .verify(&signature_data, &bundle.signed_prekey.signature)?;
        if !signature_valid {
...
mod tests {
    ...
    fn test_signal_protocol_initialization() {
        ...
        let signed_prekey = signal_protocol
            .generate_signed_prekey()
            .expect("Should generate signed pre-key");

        assert_eq!(signed_prekey.id, 1);
        assert_eq!(
            signed_prekey.device_public_key.identity_id,
            signal_protocol.identity_keypair.identity_id()
        );

        // Generate one-time pre-keys
        let one_time_prekeys = signal_protocol
            .generate_one_time_prekeys(10)
    ...
    fn test_prekey_bundle_creation_and_verification() {
        ...
        signal_protocol
            ...
            .expect("Should generate signed pre-key");
        signal_protocol
            .generate_one_time_prekeys(5)
            .expect("Should generate one-time pre-keys");

        // Create bundle
        let bundle = signal_protocol
    ...
    fn test_key_distribution_server() {
        ...
        server
            .upload_prekey_bundle(&bundle)
            .expect("Should upload bundle");

        let retrieved_bundle = server
            .get_prekey_bundle(&bundle.identity_key.identity_id, &bundle.device_id)
            .expect("Should retrieve bundle");

        assert_eq!(
            bundle.identity_key.identity_id,
            retrieved_bundle.identity_key.identity_id
        );
        assert_eq!(bundle.device_id, retrieved_bundle.device_id);

        let devices = server
            .list_devices(&bundle.identity_key.identity_id)
    ...
    fn test_key_exchange_flow() {
        ...
        let mut bob_protocol =
            SignalProtocol::new(bob_identity, b"bob_device").expect("Should create Bob's protocol");

        // Generate keys
        alice_protocol
            .generate_signed_prekey()
            .expect("Should generate Alice's signed pre-key");
        alice_protocol
            .generate_one_time_prekeys(5)
            .expect("Should generate Alice's one-time pre-keys");

        bob_protocol
            .generate_signed_prekey()
            .expect("Should generate Bob's signed pre-key");
        bob_protocol
            .generate_one_time_prekeys(5)
            .expect("Should generate Bob's one-time pre-keys");

        // Create bundles
        let alice_bundle = alice_protocol
            .create_prekey_bundle()
            .expect("Should create Alice's bundle");
        let bob_bundle = bob_protocol
            .create_prekey_bundle()
            .expect("Should create Bob's bundle");

        // Alice initiates key exchange with Bob
        let alice_result = alice_protocol
            .initiate_key_exchange(&bob_bundle)
            .expect("Should initiate key exchange");

        // Bob responds to Alice's key exchange
        let bob_result = bob_protocol
            .respond_to_key_exchange(
