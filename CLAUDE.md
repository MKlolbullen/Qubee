...
- `network/p2p_node.rs` — libp2p 0.56 node (TCP + DNS + Yamux + Noise XX + gossipsub + Kademlia + mDNS). `P2PNode` runs on its own Tokio task; the JNI layer talks to it through an mpsc command channel and forwards events back to Kotlin via JNI callbacks.
