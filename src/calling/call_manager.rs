...
use crate::calling::signaling::{SignalingServer, SignalingMessage};
...
impl CallManager {
    ...
    pub async fn new(
        ...
        let media_encryption = MediaEncryption::new(MediaKey::new(key_bytes));
    ...
    pub async fn toggle_mute(&self, call_id: CallId, participant: IdentityId) -> Result<bool> {
        let (new_state, is_muted) = {
            ...
            let call = calls.get_mut(&call_id)
                .ok_or_else(|| anyhow::anyhow!("Call not found"))?;
            let participant_info = call.participants.get_mut(&participant)
            ...
            (participant_info.media_state.clone(), participant_info.is_muted)
        };

        // Update WebRTC audio track (lock already released)
        self.webrtc_manager.set_audio_enabled(call_id, participant, !is_muted).await?;
    ...
    pub async fn toggle_video(&self, call_id: CallId, participant: IdentityId) -> Result<bool> {
        let (new_state, is_video_enabled) = {
            let mut calls = self.calls.write().await;
            let call = calls.get_mut(&call_id)
                .ok_or_else(|| anyhow::anyhow!("Call not found"))?;
            let participant_info = call.participants.get_mut(&participant)
            ...
            (participant_info.media_state.clone(), participant_info.is_video_enabled)
        };

        // Update WebRTC video track (lock already released)
        self.webrtc_manager.set_video_enabled(call_id, participant, is_video_enabled).await?;
    ...
    async fn establish_peer_connection(&self, call_id: CallId, participant: IdentityId) -> Result<()> {
        // Generate a fresh media key for this peer connection
        let mut key_bytes = [0u8; 32];
        OsRng.fill_bytes(&mut key_bytes);
        let media_key = MediaKey::new(key_bytes);
