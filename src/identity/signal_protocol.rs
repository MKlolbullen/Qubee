...
pub struct OneTimePreKey {
    ...
    #[serde(with = "crate::identity::identity_key::serde_kyber_pubkey")]
...
impl SignalProtocol {
    ...
    pub fn generate_one_time_prekeys(&mut self, count: usize) -> Result<Vec<OneTimePreKey>> {
        ...
        for _ in 0..count {
            ...
            let x25519_private = x25519_dalek::StaticSecret::random_from_rng(&mut rand::rngs::OsRng);
    ...
    pub fn initiate_key_exchange(
        ...
        ratchet.initialize_sender(&combined_secret, &remote_device.x25519_public, &remote_device.kyber_public)?;
    ...
    pub fn respond_to_key_exchange(
        ...
        let mut ratchet = EnhancedHybridRatchet::new();
        ratchet.initialize_receiver(
            &combined_secret,
            self.device_key.x25519_static_secret(),
            self.device_key.kyber_secret_key()?,
        )?;
