...
impl GroupManager {
    ...
    pub fn update_member_role(
        ...
        // Update role - collect old_role then release the borrow before log
        let old_role = if let Some(member) = group.members.get_mut(&member_id) {
            let old = member.role.clone();
        ...
        group.last_updated = now_secs()?;
        group.version += 1;
        // Release the mutable borrow on self.groups before calling log_group_event
        let _ = group;
    ...
    pub fn create_invitation(
        ...
        self.keystore.store_key(&invitation_key, &serialized, KeyType::MessageKey, bincode_metadata(serialized.len()))?;
    ...
    fn log_group_event(
        ...
        self.keystore.store_key(&event_key, &serialized, KeyType::MessageKey, bincode_metadata(serialized.len()))?;
    ...
    fn store_group_securely(&mut self, group_id: &GroupId) -> Result<()> {
        if let Some(group) = self.groups.get(group_id) {
            ...
            self.keystore.store_key(&key_name, &serialized, KeyType::EncryptionKey, bincode_metadata(serialized.len()))?;
        }
        Ok(())
    }
...
/// Build the standard `KeyMetadata` used when storing serialized bincode blobs.
fn bincode_metadata(size: usize) -> KeyMetadata {
    KeyMetadata {
