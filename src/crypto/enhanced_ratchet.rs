...
use pqcrypto_traits::kem::{SecretKey as KemSk, SharedSecret as _};
...
pub struct EnhancedHybridRatchet {
    ...
    // Post-quantum KEM ratchet state (stored as raw bytes)
    pq_send_key: Option<Zeroizing<Vec<u8>>>,
    pq_recv_key: Option<kyber768::PublicKey>,

    // Chain keys for sending and receiving
    send_chain_key: Option<Zeroizing<[u8; 32]>>,
    recv_chain_key: Option<Zeroizing<[u8; 32]>>,

    // Message counters for replay protection
    send_counter: u64,
    recv_counter: u64,

    // Chain ID counters — incremented on each ratchet step
    send_chain_id: u64,
    recv_chain_id: u64,

    // Number of messages sent on the previous chain before the last ratchet step
    previous_send_chain_length: u64,

    // Skipped message keys for out-of-order delivery
    skipped_keys: HashMap<MessageId, Zeroizing<[u8; 32]>>,
...
impl EnhancedHybridRatchet {
    ...
    pub fn initialize_sender(
        ...
        let dh_keypair = StaticSecret::random_from_rng(&mut rand::rngs::OsRng);
        // pqcrypto encapsulate returns (SharedSecret, Ciphertext)
        let (pq_shared_secret, _pq_ciphertext) = kyber768::encapsulate(remote_pq_key);
        ...
        let root_key_copy = *self.root_key;
        let (new_root_key, send_chain_key) = self.kdf_rk(
            &root_key_copy,
            &combined_secret,
        )?;

        self.root_key = Zeroizing::new(new_root_key);
        self.send_chain_key = Some(Zeroizing::new(send_chain_key));
    ...
    pub fn initialize_receiver(
        ...
        self.dh_send_key = Some(dh_keypair);
        self.pq_send_key = Some(Zeroizing::new(pq_keypair.as_bytes().to_vec()));
    ...
    pub fn dh_ratchet(&mut self, remote_public_key: &X25519PublicKey) -> Result<()> {
        ...
        let new_keypair = StaticSecret::random_from_rng(&mut rand::rngs::OsRng);
        ...
        self.root_key = Zeroizing::new(new_root_key);
        self.send_chain_key = Some(Zeroizing::new(new_chain_key));
        self.dh_send_key = Some(new_keypair);
        self.dh_recv_key = Some(*remote_public_key);

        // Save previous chain length before resetting counter
        self.previous_send_chain_length = self.send_counter;
        self.send_counter = 0;
        self.send_chain_id = self.send_chain_id.saturating_add(1);
    ...
    pub fn pq_ratchet(&mut self, remote_public_key: &kyber768::PublicKey) -> Result<()> {
        ...
        let (_pq_public_key, pq_secret_key) = kyber768::keypair();
        // pqcrypto encapsulate returns (SharedSecret, Ciphertext)
        let (shared_secret, _ciphertext) = kyber768::encapsulate(remote_public_key);
        ...
        self.root_key = Zeroizing::new(new_root_key);
        self.recv_chain_key = Some(Zeroizing::new(new_chain_key));
        self.pq_send_key = Some(Zeroizing::new(pq_secret_key.as_bytes().to_vec()));
        self.pq_recv_key = Some(remote_public_key.clone());
        self.recv_chain_id = self.recv_chain_id.saturating_add(1);
    ...
    fn get_send_message_key(&mut self) -> Result<Zeroizing<[u8; 32]>> {
        ...
        Ok(Zeroizing::new(message_key))
    }

    fn get_recv_message_key(&mut self, message_number: u64) -> Result<Zeroizing<[u8; 32]>> {
        ...
        for i in 0..skip_count {
            ...
            self.skipped_keys.insert(message_id, Zeroizing::new(message_key));
            current_chain_key = Zeroizing::new(new_chain_key);
        }
