...
pub async fn send_file<W: AsyncWriteExt + Unpin>(
    ...
    // Note: cover traffic (enable_cover) is logged but not spawned — WriteHalf is not Clone
    if enable_cover {
        let _freq = dummy_freq_secs.unwrap_or(15);
        tracing::debug!("cover traffic requested but not spawned in this build");
    }
...
pub async fn receive_file<R: AsyncReadExt + Unpin>(
    ...
    loop {
        ...
        let decrypted = cipher.decrypt(Nonce::from_slice(&chunk.nonce), chunk.ciphertext.as_slice())
            .context("chunk decrypt failed")?;

        let ephemeral_pk = dilithium2::PublicKey::from_bytes(&chunk.ephemeral_pk)?;
        let signature = dilithium2::DetachedSignature::from_bytes(&chunk.ephemeral_sig)?;
        dilithium2::verify_detached_signature(&signature, &chunk.ciphertext, &ephemeral_pk)
            .map_err(|_| anyhow::anyhow!("signature verification failed"))?;

        verify_and_pin_ephemeral_key(&ephemeral_store, sender_id, &chunk.ephemeral_pk)?;

        if chunk.is_dummy {
            tracing::trace!(seq = chunk.seq_number, "dropped dummy chunk");
            continue;
        }
        ...
        if chunk.is_hash {
            ...
            if received_hash == calculated_hash.as_bytes() {
                tracing::info!("file integrity verified");
            } else {
                tracing::error!("file integrity verification failed");
