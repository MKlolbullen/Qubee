...
pub fn decrypt_audio_packet(
    ...
    let decrypted = cipher.decrypt(Nonce::from_slice(&packet.nonce), packet.ciphertext.as_ref())
        .context("audio packet decrypt failed")?;

    let ephemeral_pk = dilithium2::PublicKey::from_bytes(&packet.ephemeral_pk)?;
    let signature = dilithium2::DetachedSignature::from_bytes(&packet.ephemeral_sig)?;
    dilithium2::verify_detached_signature(&signature, &packet.ciphertext, &ephemeral_pk)
    ...
    loop {
        ...
        if let Ok(dummy_packet) = encrypt_audio_packet(&mut r, b"", 1, seq, true, &identity_sk) {
            ...
            if udp_socket.send_to(&data, peer_addr).await.is_err() {
                tracing::trace!("failed to send dummy audio packet");
            }
        }
